...
class TestRestRequest(TestCase):
    def setUp(self):
        ...
        self.rest_request = RestRequest(self.url, rest_client=self.mock_requests)

    def test_get_sets_proper_request(self):
        self.rest_request.get(self.action, self.sample_params)
        self.mock_requests.request.assert_called_with('GET', self.proper_url, data=None, params=self.sample_params,
                                                      headers={'content-type': 'application/json'})

    def test_put_sets_proper_request(self):
        self.rest_request.put(self.action, self.sample_data)
        self.mock_requests.request.assert_called_with('PUT', self.proper_url, data=self.sample_data, params=None,
                                                      headers={'content-type': 'application/json'})

    def test_post_sets_proper_request(self):
        self.rest_request.post(self.action, self.sample_data)
        self.mock_requests.request.assert_called_with('POST', self.proper_url, data=self.sample_data, params=None,
                                                      headers={'content-type': 'application/json'})

    def test_delete_sets_proper_request(self):
        self.rest_request.delete(self.action)
        self.mock_requests.request.assert_called_with('DELETE', self.proper_url, data=None, params=None,
                                                      headers={'content-type': 'application/json'})
